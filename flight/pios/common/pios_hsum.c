...
static int32_t PIOS_HSUM_Get(uint32_t rcvr_id, uint8_t channel);
static uint16_t PIOS_HSUM_RxInCallback(uint32_t context,
                                       uint8_t *buf,
                                       uint16_t buf_len,
                                       uint16_t *headroom,
                                       bool *need_yield);
static void PIOS_HSUM_Supervisor(uint32_t hsum_id);
...
#endif /* if defined(PIOS_INCLUDE_FREERTOS) || defined(PIOS_INCLUDE_CHIBIOS) */
...
int32_t PIOS_HSUM_Init(uint32_t *hsum_id,
